...
def pipeline_gorsel_kutusu(gorseller, expanded=False):
    """Pipeline'ın ürettiği statik görselleri ilgili sekmede gösterir."""
    mevcut = []
    for dosya, baslik, aciklama in gorseller:
        yol = CHART_DIR / dosya
        if yol.exists():
            mevcut.append((yol, baslik, aciklama))

    if not mevcut:
        return

    with st.expander("🖼️ Pipeline görsel çıktıları", expanded=expanded):
        kolonlar = st.columns(2 if len(mevcut) > 1 else 1)
        for i, (yol, baslik, aciklama) in enumerate(mevcut):
            with kolonlar[i % len(kolonlar)]:
                st.markdown(f"**{baslik}**")
                st.image(str(yol), use_container_width=True)
...
if sayfa == "Özet":
    ...
    pipeline_gorsel_kutusu([
        (
            "01_funnel.png",
            "Dönüşüm Hunisi Görseli",
            "Event akışını tek görselde özetler: en büyük kayıp sayfa görüntülemeden sepete ekleme adımına geçiştedir.",
        ),
    ])
  ...
  <div class="merge-card orange">
    ...
    pipeline_gorsel_kutusu([
        (
            "00_eda.png",
            "EDA Özet Görseli",
            "Veri büyüklüğü, event dağılımı ve temel rating yapısını tek ekranda toplayan keşifsel analiz çıktısıdır.",
        ),
    ])
...
# =========================================================================
    ...
    pipeline_gorsel_kutusu([
        (
            "05_gelir.png",
            "Gelir Odaklı Ürün Analizi",
            "Ürünlerin gelir katkısını kategori ve satış sinyalleriyle birlikte okuyarak ticari performansı özetler.",
        ),
    ])
...
# =========================================================================
    ...
    pipeline_gorsel_kutusu([
        (
            "03_top_urunler.png",
            "Top Ürünler Görseli",
            "Genel öneri listesinde öne çıkan ürünleri ve bu ürünlerin kategori dağılımını gösterir.",
        ),
        (
            "04_skor.png",
            "Öneri Skoru Dağılımı",
            "Ürün skorlarının hangi aralıkta yoğunlaştığını ve önerilir eşiğinin nerede oluştuğunu gösterir.",
        ),
    ])
...
# =========================================================================
    ...
    pipeline_gorsel_kutusu([
        (
            ...
            "Modelin doğru ve yanlış sınıflandırmalarını TN, FP, FN ve TP üzerinden özetler.",
        ),
        (
            "07_roc.png",
            "ROC Eğrisi Görseli",
            "Modelin önerilir ve önerilmez sınıflarını ayırma gücünü AUC metriğiyle gösterir.",
        ),
        (
            "08_onem.png",
            "Özellik Önemi Görseli",
            "Random Forest modelinin kararlarında hangi değişkenlere daha fazla ağırlık verdiğini gösterir.",
        ),
    ])
...
# =========================================================================
    ...
    pipeline_gorsel_kutusu([
        (
            ...
            "Pozitif, nötr ve negatif yorum dağılımını; ayrıca rating sinyalinin duygu analiziyle ilişkisini gösterir.",
        ),
    ])
...
# =========================================================================
    ...
    pipeline_gorsel_kutusu([
        (
            ...
            "Müşterilerin Şampiyon, Sadık, Risk Altında, Kayıp, Yeni ve Potansiyel segmentlerine nasıl dağıldığını gösterir.",
        ),
        (
            "10_rfm_scatter.png",
            "RFM Dağılım Haritası",
            "Recency ve harcama ilişkisini segment renkleriyle gösterir; nokta yoğunluğu müşteri davranışının doğal olarak üst üste bindiği alanları anlatır.",
        ),
    ])
